...
for stup=1:bun
    ...
    y_ax8(1,stup)= average;
    y_std8(1,stup)=deviation;
end


%Plot
plot(x_ax, y_ax1, 'LineWidth',1.5)
hold on
plot(x_ax, y_ax2, 'LineWidth',1.5)
plot(x_ax, y_ax3, 'LineWidth',1.5)
plot(x_ax, y_ax4, 'LineWidth',1.5)
plot(x_ax, y_ax5, 'LineWidth',1.5)
plot(x_ax, y_ax6, 'LineWidth',1.5)
plot(x_ax, y_ax7, 'LineWidth',1.5)
plot(x_ax, y_ax8, 'LineWidth',1.5)
...
leg=legend('$(I_4,0)$', '$(\hat{P}(0), \hat{p}_i(0))$', '$x_P$ avg of $p_i^\circ$', '$(I_4, mean(\hat{p}_i(0)))$', '$(\hat{P}(0), mean(\hat{p}_i(0))$', '$(I_4, \hat{p}_i(0))$', '$(I_4, mean(\hat{p}_i(0))+ 10)$', '$(I_4, mean(\hat{p}_i(0))+ 100)$');
